...
let parseToml;
let cosmiconfig;
const loadToml = function loadToml(filepath, content) {
    parseToml = (0, load_package_util_1.loadPackage)('@iarna/toml', "fileLoader's ability to parse TOML files").parse;
    try {
...
const getSearchOptions = (options) => {
    if (options.absolutePath) {
        return {
            searchPlaces: [(0, path_1.basename)(options.absolutePath)],
            searchFrom: (0, path_1.dirname)(options.absolutePath),
        };
    }
...
const fileLoader = (options = {}) => {
    cosmiconfig = (0, load_package_util_1.loadPackage)('cosmiconfig', 'fileLoader');
    const { cosmiconfigSync } = cosmiconfig;
    return (additionalContext = {}) => {
        const { searchPlaces, searchFrom } = getSearchOptions(options);
        const loaders = Object.assign({ '.toml': loadToml }, options.loaders);
        const explorer = cosmiconfigSync('env', Object.assign(Object.assign({ searchPlaces }, options), { loaders, transform: (result) => {
                var _a, _b;
                if (!result ||
                    ((_a = options.ignoreEnvironmentVariableSubstitution) !== null && _a !== void 0 ? _a : true)) {
                    return result;
                }
                try {
                    const updatedResult = transformFileLoaderResult(result.config, Object.assign(Object.assign(Object.assign({}, additionalContext), result.config), process.env), (_b = options.disallowUndefinedEnvironmentVariables) !== null && _b !== void 0 ? _b : true);
        ...
        (0, debug_util_1.debug)(`File-loader has loaded a configuration file from ${result.filepath}`);
        return result.config;
    };
};
exports.fileLoader = fileLoader;
function resolveReference(reference, currentContext) {
...
function transformFileLoaderResult(obj, context, disallowUndefinedEnvironmentVariables, visited = new Set()) {
    if (typeof obj === 'string') {
        const match = obj.match(/\$\{(.+?)\}/g);
        if (match) {
            for (const placeholder of match) {
                const { variable, defaultValue } = extractVariableNameAndDefaultValue(placeholder);
                let resolvedValue = resolveReference(variable, context);
                if (obj === resolvedValue) {
                    throw new Error(`Circular self reference detected: ${obj} -> ${resolvedValue}`);
                }
                if (resolvedValue !== null) {
                    if (typeof resolvedValue === 'string') {
                        ...
                        if (resolvedValue.match(/(?!(?<=\\))\$\{(.+?)\}/)) {
                            try {
                                resolvedValue = transformFileLoaderResult(resolvedValue, context, disallowUndefinedEnvironmentVariables, visited);
                            }
                            catch (error) {
                                if (error instanceof RangeError) {
                                    (0, debug_util_1.debug)(`Can not resolve a circular reference in ${obj} -> ${resolvedValue} -> ${error.message}`, error);
                                }
                                throw error;
                            }
                        }
                        ...
                        obj = transformFileLoaderResult(obj, resolvedValue, disallowUndefinedEnvironmentVariables, visited);
                    }
                    ...
                    throw new Error(`Environment variable is not set for variable name: '${variable}'`);
                }
            }
        }
    }
        ...
        if (Array.isArray(obj)) {
            for (let i = 0; i < obj.length; i++) {
                obj[i] = transformFileLoaderResult(obj[i], context, disallowUndefinedEnvironmentVariables, visited);
            }
        }
        else {
            for (const key in obj) {
                obj[key] = transformFileLoaderResult(obj[key], context, disallowUndefinedEnvironmentVariables, visited);
            }
        }
        visited.delete(obj);
    }
...
function extractVariableNameAndDefaultValue(placeholder) {
    ...
    const defaultValue = splitValues.length === 1
        ? undefined
